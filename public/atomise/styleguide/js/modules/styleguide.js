...
(function(atomise){
  ...
  var updateStyleGuideState = function(rendered){
    ...
    if(built.length == countEl()){ // The styleguide is completly built
      atomise.event.send('styleguide:build:ended');
    }
  };
  ...
  var buildNavigation = function(){
    ...
    var patternsTree = atomise.logic.getPatternsTree();
  ...
  var makeHeader = function(){
    $.get(atomise.logic.getStyleguideTemplate('header'), function(template) {
  ...
  var makeContent = function(pattern){
    ...
    if(pattern !== undefined){
      ...
      // Load all patterns
      patterns = atomise.logic.getAllPatterns();
    }
    patterns = atomise.logic.sortPatterns(patterns);
  ...
  var loadPattern = function(name, category, path){
    $.get(atomise.logic.getPatternTemplate(path), function(template) {
